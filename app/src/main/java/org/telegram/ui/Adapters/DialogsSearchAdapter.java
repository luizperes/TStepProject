                            ...
                            for (String q : search) {
                                ...
                                if (found != 0) {
                                    ...
                                    if (data != null && cursor.byteBufferValue(0, data.buffer) != 0 && cursor.byteBufferValue(6, data2.buffer) != 0) {
                                        ...
                                        if (found == 1) {
                                            dialogSearchResult.name = AndroidUtilities.replaceTags(ApplicationLoader.applicationContext, "<c#ff00a60e>" + ContactsController.formatName(user.first_name, user.last_name) + "</c>");
                                        } else {
                                            dialogSearchResult.name = Utilities.generateSearchName("@" + user.username, null, "@" + q);
                                        }
                                        dialogSearchResult.object = chat;
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        ...
        if (type == 1) {
            ...
            if (i < searchResult.size()) {
                ...
                try {
                    username = AndroidUtilities.replaceTags(ApplicationLoader.applicationContext, String.format("<c#ff4d83b3>@%s</c>%s", user.username.substring(0, foundUserName.length()), user.username.substring(foundUserName.length())));
                } catch (Exception e) {
