...
public class NotificationsController {
    ...
    private void showOrUpdateNotification(boolean notifyAboutLast) {
        ...
        try {
            ...
            if ((int)dialog_id == 0 || pushDialogs.size() > 1 || AndroidUtilities.needShowPasscode(false) || UserConfig.isWaitingForPasscodeEnter) {
                name = LocaleController.getString("app_name", R.string.app_name);
                replace = false;
            } else {
                if (chat != null) {
                    name = chat.title;
    ...
    public void showWearNotifications(boolean notifyAboutLast) {
        ...
        for (long dialog_id : sortedDialogs) {
            ...
            NotificationCompat.Action action = new NotificationCompat.Action.Builder(R.mipmap.ic_reply_icon, replyToString, replyPendingIntent).addRemoteInput(remoteInput).build();
    ...
    private void sendAlertToPebble(String message) {
        try {
            ...
            data.put("title", LocaleController.getString("app_name", R.string.app_name));
            data.put("body", message);
            final JSONObject jsonData = new JSONObject(data);
            final String notificationData = new JSONArray().put(jsonData).toString();

            i.putExtra("messageType", "PEBBLE_ALERT");
            i.putExtra("sender", LocaleController.formatString("app_name", R.string.app_name));
